...
Each line represents one item and charges exactly one cost center. If an expense applies to different cost centers, the requestor enters separate lines.

Before a request can be submitted, the system should confirm that:

- all line items add up to the request total; and
- every line has one valid cost center.
